...
"""
kaiten_kk_img={
    pg.transform.flip(kk_img,-45),
    pg.transform.flip(kk_img,0),
    pg.transform.flip(kk_img,45),
    pg.transform.flip(kk_img,90),
    pg.transform.flip(kk_img,-45),
    pg.transform.flip(kk_img,0),
    pg.transform.flip(kk_img,45),
    pg.transform.flip(kk_img,90),  
...
def check_bound(rect: pg.Rect) -> tuple[bool, bool]:
    ...
    if rect.left< 0 or WIDTH < rect.right:  # 横方向判定
        yoko = False
    if rect.top-40 < 0 or HEIGHT -40 < rect.bottom:  # 縦方向判定
        tate = False
    return yoko, tate
...
def main():
    ...
    #colorかえたい
        ...
        bd_img.set_colorkey((0, 0, 0))  # 黒い部分を透明にする
        #課題5：爆弾の色を変えた
        #pg.draw.circle(bd_img, (25*tmr%255, (255-tmr)%255,(30*tmr-tmr*2)%255), (10, 10), 10)
        if kk_rct.colliderect(bd_rct): # 練習５
            pg.draw.circle(kk_img,(0,0,0),)
            print("ゲームオーバー")
            return   #ゲームオーバー 
